...
def get_user_input():
    ...
    print("Enter the transactions (comma-separated items):")
    for _ in range(num_transactions):
        transaction_input = input(f"Transaction {_ + 1}: ")
        items = transaction_input.strip().split(',')
        transaction = set(item.strip() for item in items)  # Create a set from the input items
